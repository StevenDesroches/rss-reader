...
impl FeedModel {
    ...
    pub fn insert_feed(self, feed: Feed) -> Result<Self> {
...

        {
            ...
            // let transaction = connection.transaction().map_err(|e| Error::Custom(e.to_string()))?;
            connection
            ...
            connection
                .execute("Commit", [])
                .map_err(|e| Error::Custom(e.to_string()))?;
            // transaction.commit().map_err(|e| Error::Custom(e.to_string()))?;
        }
    ...
    pub fn get_feeds(&self) -> Result<Vec<(i32, String, String)>> {
        ...
        for row in rows {
            feeds.push(row.map_err(|e| Error::Custom(e.to_string()))?);
        }
        Ok(feeds)
    }
    ...
    pub fn get_articles_for_feed(&self, feed_id: i32) -> Result<Vec<(i32, String, String)>> {
        ...
        for row in rows {
            articles.push(row.map_err(|e| Error::Custom(e.to_string()))?);
        }
        Ok(articles)
    }
}
